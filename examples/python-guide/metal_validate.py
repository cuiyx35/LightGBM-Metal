...
def compare(name: str, x_train, y_train, x_held, y_held, *, rounds: int,
            objective: str = "binary", extra: dict | None = None) -> dict:
    ...
    params = {
        ...
        "verbosity": 1, "num_threads": 2, "num_leaves": 15,
    ...
    for device in ("cpu", "metal"):
        ...
        model = lgb.train({**params, "device_type": device}, dataset,
                          num_boost_round=rounds)
        fit_seconds = time.perf_counter() - start
        prediction = model.predict(x_held)
        predictions[device] = prediction
        metric = (roc_auc_score(y_held, prediction) if objective == "binary"
                  else mean_squared_error(y_held, prediction))
        runs[device] = {
            "fit_seconds_observational": round(fit_seconds, 6),
            "metric": float(metric), "trees": model.num_trees(),
        }
        if name == "dense_numeric" and device == "metal":
            with tempfile.TemporaryDirectory(prefix="lgbm-metal-test-") as tmp:
                ...
                reload_error = float(np.max(np.abs(
                    reloaded.predict(x_held) - prediction)))
    ...
    result = {
        "case": name, "status": "PASS", "train_rows": len(y_train),
        "rounds": rounds, "cpu": runs["cpu"], "metal": runs["metal"],
...
def run_cases() -> list[dict]:
    ...
    cases.append(compare("dense_numeric", x[:12_000], y[:12_000],
                         x[12_000:], y[12_000:], rounds=30))
    ...
    mixed = pd.DataFrame(np.column_stack([numeric, sparse]),
                         columns=[f"f{i}" for i in range(16)])
    mixed["category"] = pd.Categorical(categories)
    y = ((np.nan_to_num(numeric[:, 0]) +
          0.6 * np.nan_to_num(numeric[:, 1]) * np.nan_to_num(numeric[:, 2]) +
          0.5 * (categories % 3 == 0) + 0.3 * sparse[:, 0]) > 0.2).astype(np.int8)
    cases.append(compare("mixed_missing_category_sparse_bagging",
                         mixed.iloc[:12_000], y[:12_000], mixed.iloc[12_000:],
                         y[12_000:], rounds=20,
                         extra={"feature_fraction": 0.8,
                                "bagging_fraction": 0.8, "bagging_freq": 1}))

    rng = np.random.default_rng(7)
    x = rng.normal(size=(130_000, 16)).astype(np.float32)
    y = (x[:, 0] + x[:, 1] * x[:, 2] +
         0.1 * rng.normal(size=len(x)) > 0).astype(np.int8)
    cases.append(compare("multiple_histogram_shards", x[:100_000], y[:100_000],
                         x[100_000:], y[100_000:], rounds=10))

    rng = np.random.default_rng(77)
    x = rng.normal(size=(8_000, 6)).astype(np.float32)
    y = (1.5 * x[:, 0] - 0.2 * x[:, 1] +
         rng.normal(scale=0.1, size=len(x))).astype(np.float32)
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("metal_validation.json"))
    args = parser.parse_args()
    report = {
        "scope": "synthetic_model_level_correctness_only",
        "machine": platform.machine(),
        "library_sha256": hashlib.sha256(
            Path(_find_lib_path()[0]).read_bytes()).hexdigest(),
    }
    try:
