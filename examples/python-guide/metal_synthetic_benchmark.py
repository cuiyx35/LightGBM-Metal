...
def make_data(
    rows: int, features: int, dense_features: int, seed: int, prevalence: float
    ...
    score += ((x[:, 1].astype(np.float32) - 63) * (x[:, 2].astype(np.float32) - 63)) / 1800
...
def main() -> None:
    ...
    parser.add_argument(
        "--full-scale", action="store_true", help="Use the 4.1 million-row, 512-feature benchmark preset"
    )
    ...
    if (
        args.train_rows < 100
        or args.held_rows < 100
        or args.features < 3
        or not 3 <= args.dense_features <= args.features
        or args.rounds < 1
        or args.threads < 1
    ...
    x, labels = make_data(
        args.train_rows + args.held_rows, args.features, args.dense_features, args.seed, args.positive_rate
    )
    generate_seconds = time.perf_counter() - start
    print(
        f"DATA_READY rows={len(x)} features={x.shape[1]} "
    ...
    for backend in ("cpu", "metal"):
        start = time.perf_counter()
        model = lgb.train({**params, "device_type": backend}, dataset, num_boost_round=1)
        warmups.append({"backend": backend, "seconds": round(time.perf_counter() - start, 6)})
        del model
        gc.collect()
        check_idle()

    order = ["metal", "cpu", "cpu", "metal"] if args.repeats == 2 else ["metal", "cpu"]
    runs = []
    representative = {}
    for index, backend in enumerate(order):
        check_idle()
        start = time.perf_counter()
        model = lgb.train({**params, "device_type": backend}, dataset, num_boost_round=args.rounds)
        fit_seconds = time.perf_counter() - start
        if model.num_trees() != args.rounds:
            raise RuntimeError(f"{backend} trained {model.num_trees()} trees")
        start = time.perf_counter()
        prediction = model.predict(x[args.train_rows :], num_threads=args.threads)
        ...
        run = {
            ...
            "average_precision": float(average_precision_score(labels[args.train_rows :], prediction)),
        ...
        if backend not in representative:
            ...
            if run["model_sha256"] != next(r["model_sha256"] for r in runs if r["backend"] == backend) or run[
                "prediction_sha256"
            ] != next(r["prediction_sha256"] for r in runs if r["backend"] == backend):
                raise RuntimeError(f"{backend} repeated result changed")
        runs.append(run)
        print(
            "RUN "
            + json.dumps(
                {k: run[k] for k in ("index", "backend", "fit_seconds", "predict_seconds", "average_precision", "auc")}
            ),
            flush=True,
        )
        del model
    ...
    medians = {
        backend: {
            "fit_seconds": statistics.median(r["fit_seconds"] for r in runs if r["backend"] == backend),
            "fit_plus_predict_seconds": statistics.median(
                r["fit_seconds"] + r["predict_seconds"] for r in runs if r["backend"] == backend
            ),
        }
        for backend in ("cpu", "metal")
    }
    report = {
        ...
        "warmups_not_timed": warmups,
        "run_order": order,
        "runs": runs,
        "median": medians,
        "cpu_over_metal_fit_speedup": (medians["cpu"]["fit_seconds"] / medians["metal"]["fit_seconds"]),
        "cpu_over_metal_fit_plus_predict_speedup": (
            medians["cpu"]["fit_plus_predict_seconds"] / medians["metal"]["fit_plus_predict_seconds"]
        ),
        ...
        "peak_process_rss_gib": round(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 2**30, 3),
        "limitations": [
            ...
            "The shared generation and Dataset construction times are excluded from per-backend training times.",
